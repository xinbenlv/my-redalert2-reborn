...
- [~] 可配置的 skirmish 选项（已支持资金、地图、玩家阵营、玩家颜色、玩家/AI 队伍与盟友关系、AI 数量、AI 难度、AI build order 与游戏速度；更细的多阵营席位独立配置仍未完成）
...
- [~] Desolator / Tesla Trooper / Chrono Legionnaire / GI 等特色单位（已补 GI deployable rifle infantry + Tesla Trooper 高科技反甲 shock infantry；仍缺 Desolator / Chrono Legionnaire 等更强特化步兵）
- [x] 兵种驻守建筑（garrison）
...
- [~] Terror Drone / Mirage / Prism Tank / Apocalypse / Kirov 等特色载具（已补 Apocalypse Tank + Prism Tank + Kirov Airship；仍缺 Terror Drone / Mirage 等更强特化单位）
- [x] MCV
...
- [x] 矿石与宝石（ore / gems）区分（地图现已生成独立 gems 资源块，3D 地表 / 晶体 / 小地图会把 gems 与 ore 区分显示）
- [x] 不同资源价值（gems 现在按双倍资源价值装车与回矿；AI 采矿选点会把 gem 田收益算进评分，优先跑高价值资源）
- [ ] 采矿动画 / 采矿反馈更明显
- [ ] 矿场耗尽后的地图观感变化加强
...
- [x] 取消建造 / 取消训练并退款
- [x] 拖拽排队 / 批量排队快捷键（已补 Shift+点击生产卡片一次排入 5 个单位，并会在多个生产建筑间自动分摊队列）
- [ ] 建筑放置占地预览更像 RA2
- [ ] 低电时生产受限的 UI 提示更清晰
...
## B7. 战斗系统
- [x] 基础伤害、射程、投射物、溅射
- [~] 武器类型与护甲类型克制矩阵（已把 damage profile / armor class 克制贯通到驻军建筑火力，并让 0 倍率目标真正免疫伤害；完整的 RA2 warhead/armor 系统仍未完成）
- [~] 反步兵 / 反甲 / 反空 / 反建筑差异化（基础单位与防御塔现在有明确 damage profile，且 Battle Bunker 会继承驻军兵种的克制关系；更多武器子类型与更细分护甲层级仍未完成）
- [ ] 开火音效与命中特效分型
- [ ] 爆炸大小 / 焦土 / 残骸
- [ ] 载具转向与炮塔旋转更真实
- [ ] 单位开火准备 / 攻击前摇 / 移动开火限制
...
- [x] 可玩的多张遭遇战地图
- [ ] 桥梁 / 可破坏桥梁
- [~] 建筑驻守点 / 城市地图元素（已补地图内中立 Battle Bunker + Civilian Block + Civilian Tower 驻守点，开始有更像样的中心城区争夺；更完整的城市建筑群、多样立面与桥梁仍未完成）
- [~] 中立建筑（已补可占领/可驻守的 Civilian Block / Civilian Tower 中立建筑，仍缺更多非驻守型地图中立设施）
- [ ] 地图装饰物（树、路灯、残骸、围栏、道路）
...
- [x] 科技攀升（AI 现在会沿 build order 继续爬完整中高科技链：空军流会在 Airfield 后继续补 Battle Lab 与 Advanced Power Plant，装甲/均衡流会在 Battle Lab 后继续补 Airfield 与 Advanced Power Plant，不再卡死在单一路线）
...
- [ ] 海空军使用能力
- [x] 难度等级（Easy / Medium / Hard 现在不再只是 UI 选项：会真实改变 AI 的决策频率、出击阈值、维修/止损预算、tech greed 节奏，以及防空/防御扩张强度）

## B14. 多人/遭遇战外壳
- [x] 多 AI 玩家支持
- [x] 2v2 / free-for-all 基础结构（已补 1vN 多 AI 混战、显式玩家/AI 队伍分配、盟友关系与按敌对队伍判定的胜负/索敌基础结构）
- [~] 阵营与颜色选择（已支持玩家阵营切换 + 玩家颜色选择；AI/盟友独立颜色与队伍关系仍未完成）
- [ ] 地图选择界面
- [x] AI 难度选择
- [x] 游戏速度选项
- [ ] 观察模式 / replay（长期项）
...
- [x] more maps
- [~] walls / bridges / garrison（已补 walls + Battle Bunker / Civilian Block / Civilian Tower infantry garrison + 地图内中立城市驻守点，bridges 与更完整的城市群仍未完成）
